...
def default_config_path():
    ...
    if env:
        return env
    near_repo = Path(__file__).resolve().parent.parent / "configs" / \
        "watch_topics.json"
    if near_repo.exists():
        return str(near_repo)
    return str(Path.home() / ".cache" / "camoufox-research" /
               "watch_topics.json")
...
def find_last_by_topic(topic):
    ...
    with cc._db() as con:
        row = con.execute(
            "SELECT id FROM campaigns WHERE topic=? ORDER BY "
            "updated_ts DESC LIMIT 1", (topic,)).fetchone()
...
def main():
    ...
    if not os.path.exists(args.config):
        print(f"конфига нет: {args.config} — заполни по configs/"
              "watch_topics.example.json")
        return
    ...
    for t in topics:
        ...
        if old:
            ...
            if args.dry:
                continue
            print(cc.start(topic, queries=t.get("queries"),
                           feeds=t.get("feeds"),
                           target_sources=int(t.get("target_sources", 4)),
                           background=False).splitlines()[0])
