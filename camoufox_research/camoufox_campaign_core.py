...
_ANGLE_SUFFIXES = (" best practices", " how it works", " problems", " alternatives")
...
_SCHEMA = """CREATE TABLE IF NOT EXISTS campaigns (
        id TEXT PRIMARY KEY,
        topic TEXT NOT NULL,
...
def _db():
    ...
    if not _DDL_DONE:
        ...
        if "feeds" not in cols:
            con.execute("ALTER TABLE campaigns ADD COLUMN feeds TEXT DEFAULT '[]'")
        scol = {r[1] for r in con.execute("PRAGMA table_info(campaign_sources)")}
        if "digest" not in scol:  # выжимка (пост-обработка охоты)
            con.execute("ALTER TABLE campaign_sources ADD COLUMN digest TEXT DEFAULT ''")
        if "live" not in scol:  # verified: -1/1/0 (жив-или-кэш/битый)
            con.execute("ALTER TABLE campaign_sources ADD COLUMN live INTEGER DEFAULT -1")
        _DDL_DONE = True
    return con
        ...
        for row in payload.get("sources", []):
            ...
            cur = con.execute(
                ...
                (
                    ...
                    row.get("tier", 2),
                    row.get("tier_label", ""),
                    (row.get("snippet") or "")[:200],
                    time.time(),
                ),
            )
            fresh += cur.rowcount
        total, uniq = con.execute(
            "SELECT COUNT(*), COUNT(DISTINCT domain) FROM campaign_sources WHERE camp_id=?",
            (camp_id,),
        ).fetchone()
...
def _finish(camp_id, topic, status, total, uniq, target, notes, done_path):
    ...
    Path(done_path).write_text(json.dumps(marker, ensure_ascii=False, indent=1), encoding="utf-8")
...
def hunt(camp_id, topic, queries, target, dl, log_path, done_path, feeds=None):
    ...
    try:
        ...
        for i, wq in enumerate(waves, 1):
            ...
            notes.append(
                f"волна{i}:+{fresh} новых ({uniq}/{target} доменов)"
...
def _target_of(camp_id):
    with _db() as con:
        row = con.execute("SELECT target_sources FROM campaigns WHERE id=?", (camp_id,)).fetchone()
